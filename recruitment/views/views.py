...
def recruitment(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Automatically set company_id from the logged-in user's employee
            if hasattr(request.user, 'employee_get') and request.user.employee_get:
                recruitment_obj.company_id = request.user.employee_get.employee_work_info.company_id
...
def open_recruitments_api(request):
    """
    API endpoint to return open job listings as JSON
    """
    ...
    for recruitment in recruitments:
        job_data = {
            "id": recruitment.id,
            "title": recruitment.title or (recruitment.job_position_id.job_position if recruitment.job_position_id else "Job Opening"),
            "positions": recruitment.open_positions.count(),
            "description": recruitment.description or "",
            "company": recruitment.company_id.company if recruitment.company_id else "Our Company",
        }
...
def application_form(request):
    ...
    if recruitment_id:
        try:
            recruitment = Recruitment.objects.get(id=recruitment_id, is_active=True, closed=False, is_published=True)
    ...
    if request.POST:
        ...
        if form.instance.recruitment_id:
            form.fields["job_position_id"].queryset = (
                form.instance.recruitment_id.open_positions.all()
            )
    else:
        # Pre-populate recruitment if provided in URL
        if recruitment:
            form.fields["recruitment_id"].initial = recruitment.id
    ...
    return render(request, "candidate/application_form.html", context)
